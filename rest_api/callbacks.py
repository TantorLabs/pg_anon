...
from rest_api.logging_context import operation_logging_context, webhook_logger
...
async def send_webhook(url: str, response_body: BaseModel, extra_headers: Optional[Dict[str, str]] = None,
    ...
    webhook_logger.info(f'Starting webhook request to {url} with payload: {json.dumps(payload, ensure_ascii=False, default=str)}')
            ...
            try:
                async with session.post(url, json=payload, ssl=verify_ssl, headers=headers) as response:
                    if response.status < 500:
                        webhook_logger.info(f'Webhook successfully sent. Status code: {response.status}')
                        return
                    else:
                ...
                webhook_logger.warning(f'Webhook attempt {attempt + 1} to {url} failed. Request failed: {e}')

            if attempt < max_retries - 1:
                delay = base_delay * (2 ** attempt)
                webhook_logger.info(f'Retrying in {delay:.2f} seconds...')
                await asyncio.sleep(delay)

    webhook_logger.error(f'All {max_retries} webhook attempts to {url} have failed')
