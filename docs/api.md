...
| internal_operation_id | string                                   | No       | Internal pg_anon operation ID generated automatically. Present only when `status` is `success` or `error`. Can be used to correlate the webhook event with operation data and logs stored in the `pg_anon_runs/` directory. Also used in the `Operation` endpoints. |
