...
| web_debug             | boolean                                           | No       | Enables writing webhook logs to the operation's log file in the `runs` directory. Default: `false`.                                                          |
...
| internal_operation_id | string                                   | No       | Internal pg_anon operation ID generated automatically. Present for all statuses. Can be used to correlate the webhook event with operation data and logs stored in the `/runs` directory. Also used in the `Operation` endpoints. |
