...
| internal_operation_id | string                                   | No       | Internal pg_anon operation ID generated automatically. Present for all statuses. Can be used to correlate the webhook event with operation data and logs stored in the `pg_anon_runs/` directory. Also used in the `Operation` endpoints. |
...
| schema_names                         | array of string                                   | No       | Schema names to keep in the dump. Other schemas are left out. See [Exclude schemas](operations/dump.md#exclude-schemas). |
| schema_masks                         | array of string                                   | No       | Regular expressions that select the schemas to keep in the dump. See [Exclude schemas](operations/dump.md#exclude-schemas). |
| exclude_schema_names                 | array of string                                   | No       | Schema names to leave out of the dump. See [Exclude schemas](operations/dump.md#exclude-schemas). |
| exclude_schema_masks                 | array of string                                   | No       | Regular expressions that select the schemas to leave out of the dump. See [Exclude schemas](operations/dump.md#exclude-schemas). |
...
| pg_dump_options                      | string                                            | No       | Additional options passed to `pg_dump` as is, at your own risk. Example: `"--no-comments --encoding=LATIN1"`. See [Options for pg_dump](operations/dump.md#options-for-pg_dump).                                                                                                                                                                                            |
...
| Field                 | Type    | Required | Description                                                                                                                                                                                                                               |
|-----------------------|---------|----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| operation_id          | string  | Yes      | External operation ID. Copied from the `operation_id` of the original dump request.                                                                                                                                                       |
| internal_operation_id | string  | No       | Internal pg_anon operation ID generated automatically. Present for all statuses. Can be used to correlate the webhook event with operation data and logs stored in the `pg_anon_runs/` directory. Also used in the `Operation` endpoints. |
| status_id             | integer | Yes      | Numeric status code. Possible values: `2` — success, `3` — error, `4` — in_progress.                                                                                                                                                      |
| status                | string  | Yes      | Human-readable status. Possible values: `success`, `error`, `in_progress`.                                                                                                                                                                |
| webhook_metadata      | Any     | No       | Metadata payload passed “as is”. Copied from the `webhook_metadata` field of the scan request.                                                                                                                                            |
...
| schema_names                         | array of string                                   | No       | Schema names to keep in the restore. Other schemas are left out. See [Exclude schemas](operations/restore.md#exclude-schemas). |
| schema_masks                         | array of string                                   | No       | Regular expressions that select the schemas to keep in the restore. See [Exclude schemas](operations/restore.md#exclude-schemas). |
| exclude_schema_names                 | array of string                                   | No       | Schema names to leave out of the restore. See [Exclude schemas](operations/restore.md#exclude-schemas). |
| exclude_schema_masks                 | array of string                                   | No       | Regular expressions that select the schemas to leave out of the restore. See [Exclude schemas](operations/restore.md#exclude-schemas). |
| pg_restore_path                      | string                                            | No       | Path to the `pg_restore` Postgres tool. Default: `/usr/bin/pg_restore`.                                                                                                                                                                                                                           |
| pg_restore_options                   | string                                            | No       | Additional options passed to `pg_restore` as is, at your own risk. Example: `"--no-comments --no-table-access-method"`. See [Options for pg_restore](operations/restore.md#options-for-pg_restore).                                                                                                                                                                                  |
...
| Field                 | Type    | Required | Description                                                                                                                                                                                                                               |
|-----------------------|---------|----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| operation_id          | string  | Yes      | External operation ID. Copied from the `operation_id` of the original restore request.                                                                                                                                                    |
| internal_operation_id | string  | No       | Internal pg_anon operation ID generated automatically. Present for all statuses. Can be used to correlate the webhook event with operation data and logs stored in the `pg_anon_runs/` directory. Also used in the `Operation` endpoints. |
| status_id             | integer | Yes      | Numeric status code. Possible values: `2` — success, `3` — error, `4` — in_progress.                                                                                                                                                      |
| status                | string  | Yes      | Human-readable status. Possible values: `success`, `error`, `in_progress`.                                                                                                                                                                |
| webhook_metadata      | Any     | No       | Metadata payload passed “as is”. Copied from the `webhook_metadata` field of the scan request.                                                                                                                                            |
...
#### Description
Deletes the operation data directory in `pg_anon_runs/`.
