...
async def test_partial_restore_creates_extension_home_schema(
    ...
    schemas = await db_manager.fetch(target_db, "SELECT nspname FROM pg_namespace WHERE nspname IN ('app','trgm_home')")
    schema_names = {r["nspname"] for r in schemas}
    assert "trgm_home" in schema_names, "extension home schema must be created before CREATE EXTENSION"
    assert "app" in schema_names, "user-include schema must be present"

    other_tables = await db_manager.fetch(
        target_db,
        """
    ...
    assert not other_tables, f"tables outside the include filter leaked through: {[r['relname'] for r in other_tables]}"

    ext_loc = await db_manager.fetch(
        target_db,
        """
    ...
    sim = await db_manager.fetch(target_db, "SELECT trgm_home.similarity('alice', 'alise') AS s")
    assert 0 < sim[0]["s"] <= 1, "pg_trgm.similarity must be callable from trgm_home"

    rows = await db_manager.fetch(target_db, "SELECT count(*) AS c FROM app.users")
    src_rows = await db_manager.fetch(source_db, "SELECT count(*) AS c FROM app.users")
