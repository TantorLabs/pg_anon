...
class BasicUnitTest:
    ...
    async def init_stress_env(self) -> None:
        ...
        schema_exists = await db_conn.fetch("""select nspname from pg_namespace where nspname = 'stress'""")
        await db_conn.close()
    ...
    async def check_rows(self, options: RunOptions, schema: str, table: str, fields: list | None, rows: list) -> bool:
        ...
        db_conn = await create_connection(ctx.connection_params)
        if fields is None:
            db_rows = await db_conn.fetch(f"""select * from "{schema}"."{table}" limit 10000""")
        else:
        ...
        for v in rows:
            ...
            for db_row in db_rows_prepared:
                if cmp_two_rows(v, db_row):
                    ...
                    break
            if not found:
                print(f"check_rows: row {v} not found")
                await db_conn.close()
                result = False
    ...
    async def check_rows_count(self, options: RunOptions, objs: list) -> bool:
        ...
        for obj in objs:
            try:
                db_rows = await db_conn.fetch(f"""select count(1) from "{obj[0]}"."{obj[1]}" """)
                if db_rows[0][0] != obj[2]:
                    failed_objs.append(obj)
                    print(f"check_rows_count: failed check {obj}, count is {db_rows[0][0]:d}")
            except Exception:
                print(exception_helper(show_traceback=True))
                failed_objs.append(obj)
                print(f"check_rows_count: failed check {obj}")
    ...
    async def check_list_tables(self, options: RunOptions, expected_tables_list: list) -> bool:
        ...
        for v in not_found_tables:
            print(f"check_list_tables: Table {v} not found!")
        for v in [x for x in db_rows if x not in expected_tables_list]:
            print(f"check_list_tables: Found unexpected table {v}!")
    ...
    async def get_list_tables_with_diff_data(self, source_options: RunOptions, target_options: RunOptions) -> tuple:
        ...
        for row in db_source_rows:
            data_query = f"""SELECT "{row[2]}" FROM "{row[0]}"."{row[1]}" LIMIT 5"""
    ...
    def test_expected_results_path(self) -> str:
        return str(Path.cwd() / "tests_old" / "expected_results")

    @staticmethod
    def get_test_dict_path(dict_name: str, output: bool = False) -> str:
        dict_type = "output" if output else "input"
        return str(Path.cwd() / "tests_old" / f"{dict_type}_dict" / dict_name)

    def get_test_expected_dict_path(self, dict_name: str) -> str:
        return str(Path(self.test_expected_results_path) / dict_name)

    @staticmethod
    def get_test_output_path(dir_name: str) -> str:
        return str(Path.cwd() / "tests_old" / "output" / dir_name)

    def save_and_compare_result(self, file_name: str, list_objects: list) -> bool:
        saved_results_file_path = Path.cwd() / "tests_old" / "saved_results"
        saved_results_file_path.mkdir(parents=True, exist_ok=True)
        saved_results_file = str(saved_results_file_path / file_name)

        expected_results_file = str(Path(self.test_expected_results_path) / (file_name + ".result"))
        to_json(list_objects, formatted=True)

        with Path(saved_results_file).open("w") as out_file:
            out_file.write(to_json(list_objects, formatted=True))

        try:
            orig_data = Path(expected_results_file).open()
        ...
        try:
            current_data = Path(saved_results_file).open()
...
class PGAnonUnitTest(unittest.IsolatedAsyncioTestCase, BasicUnitTest):
    ...
    async def test_07_sync_data(self) -> None:
        ...
        self.assertTrue(await self.check_rows(options, "schm_mask_include_1", "tbl_123", None, rows))
...
class PGAnonRestoreCleanTest(unittest.IsolatedAsyncioTestCase, BasicUnitTest):
    ...
    async def test_02_dump_and_restore_with_clean_db(self) -> None:
        ...
        await self.make_dump(self.test_source_db, output_dir)
        restore_options = await self.make_restore(output_dir)
...
class PGAnonPartialDumpRestoreUnitTest(unittest.IsolatedAsyncioTestCase, BasicUnitTest):
    ...
    async def test_02_partial_dump_include_restore_full(self) -> None:
        ...
        output_dir = self.get_test_output_path(
            "PGAnonPartialDumpRestoreUnitTest.test_02_partial_dump_include_restore_full"
        )
    ...
    async def test_03_partial_dump_exclude_restore_full(self) -> None:
        ...
        output_dir = self.get_test_output_path(
            "PGAnonPartialDumpRestoreUnitTest.test_03_partial_dump_exclude_restore_full"
        )
    ...
    async def test_04_partial_dump_include_exclude_restore_full(self) -> None:
        ...
        partial_tables_exclude_dict_file = self.get_test_dict_path("test_partial_exclude_tables_dict.py")
        output_dir = self.get_test_output_path(
            "PGAnonPartialDumpRestoreUnitTest.test_04_partial_dump_include_exclude_restore_full"
        )
    ...
    async def test_05_partial_dump_include_restore_exclude(self) -> None:
        ...
        partial_tables_exclude_dict_file = self.get_test_dict_path("test_partial_exclude_tables_dict.py")
        output_dir = self.get_test_output_path(
            "PGAnonPartialDumpRestoreUnitTest.test_05_partial_dump_include_restore_exclude"
        )
    ...
    async def test_06_partial_dump_exclude_restore_include(self) -> None:
        ...
        partial_tables_exclude_dict_file = self.get_test_dict_path("test_partial_exclude_tables_dict.py")
        output_dir = self.get_test_output_path(
            "PGAnonPartialDumpRestoreUnitTest.test_06_partial_dump_exclude_restore_include"
        )
    ...
    async def test_07_partial_dump_full_restore_include_exclude(self) -> None:
        ...
        partial_tables_exclude_dict_file = self.get_test_dict_path("test_partial_exclude_tables_dict.py")
        output_dir = self.get_test_output_path(
            "PGAnonPartialDumpRestoreUnitTest.test_07_partial_dump_full_restore_include_exclude"
        )
        ...
        passed_stages.append("test_07_partial_dump_full_restore_include_exclude")
...
class PGAnonDictGenUnitTest(unittest.IsolatedAsyncioTestCase, BasicUnitTest):
    def __init__(self, *args, **kwargs) -> None:  # noqa: ANN002, ANN003
        ...
        self.target_no_sens_dict_expected = self.get_test_expected_dict_path(
    ...
    def assert_sens_dicts(self, prepared_sens_dict: str | Path, prepared_sens_dict_expected: str | Path) -> None:
        ...
        with (
            ...
            expected_result_list_of_iterate_dict.extend(iterate_dict_level_1(d2))
    ...
    def assert_no_sens_dicts(
        ...
        with (
            Path(prepared_no_sens_dict).open(encoding="utf-8") as file1,
            Path(prepared_no_sens_dict_expected).open(encoding="utf-8") as file2,
        ):
            d1 = json.load(file1)
            d2 = json.load(file2)
            ...
            for d1_field, d2_field in zip(sorted_d1, sorted_d2, strict=True):
    ...
    async def test_02_create_dict(self) -> None:
        ...
        res = await PgAnonApp(options).run()
        self.assertTrue(Path(self.target_sens_dict).exists())
        self.assertTrue(Path(self.target_no_sens_dict).exists())
    ...
    async def test_04_restore(self) -> None:
        ...
        self.assertTrue(await self.check_rows(options, "schm_mask_ext_exclude_2", "card_numbers", None, rows))
        ...
        not_found_in_target = await self.check_list_tables_and_fields(self.options["dump"], self.options["restore"])
    ...
    async def test_05_repeat_create_dict_with_no_sens_dict(self) -> None:
        ...
        res = await PgAnonApp(options).run()
        self.assertTrue(Path(self.target_sens_dict).exists())
        self.assertTrue(Path(target_no_sens_dict_repeat).exists())
    ...
    async def test_07_create_dict_using_include_rules(self) -> None:
        ...
        prepared_sens_dict_expected = self.get_test_expected_dict_path(
        ...
        res = await PgAnonApp(options).run()
        self.assertTrue(Path(prepared_sens_dict).exists())
        self.assertTrue(Path(self.target_no_sens_dict).exists())
    ...
    async def test_08_create_dict_using_include_and_skip_rules_with_masks(self) -> None:
        ...
        prepared_sens_dict = self.get_test_dict_path(
            "test_prepared_sens_dict_result_by_include_and_skip_rules.py", output=True
        )
        prepared_sens_dict_expected = self.get_test_expected_dict_path(
    ...
    async def test_09_create_dict_using_partial_constants(self) -> None:
        ...
        prepared_sens_dict = self.get_test_dict_path(
            "test_prepared_sens_dict_result_by_partial_constants.py", output=True
        )
        prepared_sens_dict_expected = self.get_test_expected_dict_path(
    ...
    async def test_10_create_dict_using_data_sql_condition(self) -> None:
        ...
        prepared_sens_dict = self.get_test_dict_path(
            "test_prepared_sens_dict_result_by_data_sql_condition.py", output=True
        )
        prepared_sens_dict_expected = self.get_test_expected_dict_path(
    ...
    async def test_13_create_dict_with_type_aliases_complex(self) -> None:
        ...
        prepared_sens_dict = self.get_test_dict_path(
            "test_prepared_sens_dict_result_type_aliases_complex.py", output=True
        )
        prepared_sens_dict_expected = self.get_test_expected_dict_path(
    ...
    async def test_15_create_dict_using_words_and_phrases_constants(self) -> None:
        ...
        prepared_sens_dict = self.get_test_dict_path(
            "test_prepared_sens_dict_result_by_words_and_phrases_constants.py", output=True
        )
        prepared_sens_dict_expected = self.get_test_expected_dict_path(
    ...
    async def test_19_create_dict_using_not_existing_functions(self) -> None:
        ...
        prepared_sens_dict = self.get_test_dict_path(
            "test_prepared_sens_dict_result_by_not_existing_functions_in_datafunc.py", output=True
        )
    ...
    async def test_20_create_dict_using_data_func_per_field(self) -> None:
        ...
        prepared_sens_dict = self.get_test_dict_path(
            "test_prepared_sens_dict_result_by_data_func_per_field.py", output=True
        )
        prepared_sens_dict_expected = self.get_test_expected_dict_path(
...
class PGAnonMaskUnitTest(unittest.IsolatedAsyncioTestCase, BasicUnitTest):
    ...
    async def test_03_mask_restore(self) -> None:
        ...
        self.assertTrue(await self.check_rows(options, "public", "contracts", ["id", "amount"], rows))

        rows = [[1, round(Decimal(202020), 2)], [2, round(Decimal(202020), 2)]]
        self.assertTrue(await self.check_rows(options, "public", "tbl_100", ["id", "amount"], rows))
...
class PGAnonViewFieldsUnitTest(unittest.IsolatedAsyncioTestCase, BasicUnitTest):
    ...
    async def test_02_view_fields_full(self) -> None:
        ...
        self.assertEqual(
            all_rows_count - excluded_fields_count, fields_counters["in_dict"] + fields_counters["not_in_dict"]
        )
    ...
    async def test_08_view_fields_with_only_sensitive_fields(self) -> None:
        ...
        executor_only_sensitive_fields_set: set[str] = {str(field) for field in executor_only_sensitive.fields}
